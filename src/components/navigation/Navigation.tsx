...
export default function Navigation() {
  ...
  // Fetch courses and verify auth on mount
  useEffect(() => {
    const initializeNav = async () => {
      try {
        // Only set loading if courses aren't loaded yet
        if (courses.length === 0) setLoading(true);

        const [coursesResponse, profileResponse] = await Promise.allSettled([
          api.get('/courses/'),
          user ? api.get('/accounts/profile/') : Promise.resolve(null)
        ]);

        // Handle courses response
        if (coursesResponse.status === 'fulfilled' && coursesResponse.value.data?.courses) {
          setCourses(coursesResponse.value.data.courses);
          setError(null); // Clear error on success
        } else if (coursesResponse.status === 'rejected') {
          setError(coursesResponse.reason instanceof Error ? coursesResponse.reason.message : "Failed to fetch courses");
          console.error("Error fetching courses:", coursesResponse.reason);
        }

        // Handle profile/auth verification
        ...
        console.error("Unexpected error during navigation initialization:", err);
      } finally {
        setLoading(false);
      }
    };
    ...
    // Listen for reconnection to automatically retry
    window.addEventListener('online', initializeNav);
    
    return () => {
      window.removeEventListener('online', initializeNav);
    };
