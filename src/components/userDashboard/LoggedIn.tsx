...
export default function LoggedIn() {
  const [data, setData] = useState<LoggedInData>({
    ...
    sections: ["courses", "certificates", "ai-labs"],
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        ...
        // Define requests
        const dashboardPromise = api.get("/accounts/dashboard/");
        const certificatesPromise = api.get("/accounts/certificates/").catch(err => {
          console.warn("Failed to fetch certificates, using mock data:", err);
          return { data: { success: false, certificates: [] } };
        });
        const recentWatchPromise = api.get("/accounts/recent-watch/").catch(err => {
          console.warn("Failed to fetch recent watch:", err);
          return { data: { success: false, recent_watch: null } };
        });

        // Execute all requests in parallel
        const [dashboardResponse, certificatesResponse, recentWatchResponse] = await Promise.all([
          dashboardPromise,
          certificatesPromise,
          recentWatchPromise
        ]);
        ...
        const recentWatchData = recentWatchResponse.data;

        // Process Recent Watch Data
        if (recentWatchData.success && recentWatchData.recent_watch) {
          setRecentWatch(recentWatchData.recent_watch);
        } else {
          setRecentWatch(null);
        }

        // Process Dashboard and Courses Data
        if (dashboardData.success && dashboardData.courses?.courses_list) {
          ...
          const updatedData: LoggedInData = {
            ...
            courses: { title: "Courses", description: "View all your courses and track its progress" },
              ...
              aiLabs: { title: "AI Labs", description: "Experiment in our labs" }
            }
          };
    ...
    // Re-fetch logic for focus and visibility (only for recent watch)
    const handleVisibilityChange = () => {
      if (!document.hidden) {
        api.get("/accounts/recent-watch/").then(res => {
          if (res.data.success) setRecentWatch(res.data.recent_watch);
        }).catch(err => console.warn("Background recent-watch fetch failed", err));
      }
    };
            ...
            <div className="px-6 md:px-8 w-[100vw] sm:w-[100vw] md:w-[90vw] lg:w-[100vw] mx-auto lg:px-10 xl:px-36 2xl:px-48">
              ...
              <div className="flex flex-row items-start justify-between mb-6 gap-4">
                <div className="flex-1">
                  <h1 className="text-2xl md:text-3.5xl mt-10 lg:text-4xl text-white font-normal tracking-tight line-clamp-2">
                    Welcome, {data.user.name}
                  </h1>
                  <p className="text-white/80 text-md mt-4 ">
            ...
            {activeSection === "courses" && (
              <>
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
                  <div className="flex-1">
                    <h2 className="text-xl md:text-2xl text-[#1a212f] font-normal tracking-tight">
              ...
              <>
                ...
                <div className="mb-10">
                  ...
                  <h2
                    ...
                    className="text-xl md:text-2xl font-normal text-[#1a212f]">
