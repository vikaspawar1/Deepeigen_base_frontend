...
export default function Profile() {
  ...
  const handleUpdateProfile = async (updatedData: Partial<ProfileData> | ProfileData) => {
    // If we already have the full profile data (e.g. from EditProfile response),
    // just update the state and switch back.
    if ('id' in updatedData && 'email' in updatedData) {
    ...
    try {
      setLoading(true);
      const response = await updateProfile(updatedData as Partial<ProfileData>);
